...
function handleKeyPress(e) {
  ...
  if (appState.dialog.id !== null) {
    ...
    if (key === "Enter") {
      ...
      if (appState.dialog.id === "confirm-guesses") cancelGuesses();
      else if (appState.dialog.id === "confirm-bets") cancelBets();
      else cancelDialog();
    }
...
function isGameOver() {
  ...
  if (config.numRounds !== null && state.round >= config.numRounds) return true;
...
function updateStatus() {
  ...
  else roundElem.innerHTML = "Round " + state.round;
...
function updateRankings() {
  ...
  for (let i = 0; i < players.length; i++) {
    if (players[i].cash !== prevCash) curRank = i + 1;
...
function updatePlayerList() {
  ...
  for (let player of players) {
    ...
    output +=
      "onclick=\"loadDialog('edit-player', " + player.id + ')">edit</a>';
    output += '<a class="game-icon material-icons" ';
    output +=
      "onclick=\"loadDialog('remove-player', " + player.id + ')">delete</a>';
    output += "</span></div>";
    output += '<div class="player-stats">';
    output +=
      '<span class="player-rank">' + rankToString(player.rank) + "</span>";
...
function currentBetLimit() {
  ...
  if (config.numRounds === null) return config.betLimit;
  else
...
function updateGuessDialog() {
  ...
  if (player.guess === null) guessInput.value = "";
  else guessInput.value = player.guess;

  guessInput.select();

  document.getElementById("guess-dialog-turn-indicator").innerHTML =
    player.name + "'s Turn";
...
function startBetting() {
  ...
  for (let player of players) {
    ...
    else state.guessList[guessIndex].players.push(player.name);
  }
...
function calculatePayouts() {
  ...
  if (count % 2 === 0) {
    // Even number of guesses (or odd excluding Elvis)
    ...
    // Odd number of guesses (even number excluding Elvis)
...
function updateBetDialog() {
  ...
  document.getElementById("bet-dialog-turn-indicator").innerHTML =
    player.name + "'s Turn";
  document.getElementById("bet-dialog-player-name").innerHTML = player.name;
  document.getElementById("bet-dialog-bet-number").innerHTML = firstBet
    ? "first"
    : "second";

  let pointValue = firstBet
    ? wwApp.config.wagerChipValue1
    : wwApp.config.wagerChipValue2;
  let pointDisplay = "";
  if (pointValue !== 0) {
    pointDisplay = " (worth " + pointValue + " point";
    if (pointValue !== 1) pointDisplay += "s";
    pointDisplay += ")";
  }
  document.getElementById("bet-dialog-bet-points").innerHTML = pointDisplay;

  let betLimit = currentBetLimit();
  if (betLimit === null || player.cash < betLimit) betLimit = player.cash;

  if (!firstBet && player.bet1Amount !== null) betLimit -= player.bet1Amount;
  ...
  let additionalBetElem = document.getElementById(
    "bet-dialog-additional-bet-amount"
  );
  let betAmountInput = document.getElementById("additional-bet-amount");
  betAmountInput.value = "";
  if (betLimit !== 0) {
    additionalBetElem.style.display = "block";
    if (betAmount !== null) betAmountInput.value = betAmount;
  } else {
...
function updateWagerBoard() {
  ...
  for (let i = 0; i < guessList.length; i++) {
    ...
    if (selected === i) output += " selected-guess";

    output += '" id="guess' + i + '" onclick="selectGuess(' + i + ')">';
    output +=
      '<span class="guess-payout">Pays ' + guessList[i].payout + " to 1</span>";
    output += '<span class="guess-value">';
    if (guessList[i].value === null) output += "&minus;&infin;";
    else output += guessList[i].value;
    output += "</span>";
...
function placeBet() {
  ...
  let betAmount = Number.parseInt(
    document.getElementById("additional-bet-amount").value
  );
  ...
  if (maxBet === null || maxBet > player.cash) maxBet = player.cash;
  if (!firstBet && player.bet1Amount !== null) maxBet -= player.bet1Amount;
  if (betAmount > maxBet) {
...
function confirmBets() {
  ...
  for (let player of players) {
    ...
    if (value1 === null) value1 = "&minus;&infin;";
    let value2 = guessList[player.bet2Guess].value;
    if (value2 === null) value2 = "&minus;&infin;";

    output += "<tr><td>" + player.name + "</td><td>";
    output += value1;
    if (showAmounts) output += " ($" + player.bet1Amount + ")";
    output += "</td><td>";
    output += value2;
    if (showAmounts) output += " ($" + player.bet2Amount + ")";
    output += "</td></tr>";
  }
  output += "</table>";
...
function revealAnswer() {
  wwApp.gameState.answer = Number.parseInt(
...
function showResults() {
  ...
  for (let i = 1; i < state.guessList.length; i++) {
    ...
    if (value <= answer) winningGuess = i;
    else break;
  }
  ...
  for (let player of state.players) {
    ...
    if (player.guess === winningValue) {
      ...
      if (winningValue === answer) netChange += wwApp.config.exactAnswerBonus;
    }
    ...
    if (player.bet1Guess === winningGuess) {
      ...
      if (player.bet1Amount !== null) payout += player.bet1Amount;
    ...
    if (player.bet2Guess === winningGuess) {
      ...
      if (player.bet2Amount !== null) payout += player.bet2Amount;
  ...
  for (let pResult of playerResults) {
    ...
    results += '<td class="';
    if (pResult.netChange > 0) results += 'positive-cash">$+';
    else if (pResult.netChange < 0) results += 'negative-cash">$';
    else results += 'zero-cash">$';
...
function showVictory() {
  ...
  for (let player of players) {
    if (player.rank === 1) winners.push(player.name);
  }
  ...
  if (winners.length === 1)
    ...
    for (let i = 0; i < winners.length; i++) {
      heading += " ";
      heading += winners[i];
      if (winners.length > 2 && i < winners.length - 1) heading += ",";
      if (i === winners.length - 2) heading += " and";
    }
...
function loadDialog(id, playerId = null) {
  ...
  let player = null;
  if (playerId !== null) player = getPlayer(playerId);
...
function applyDialog() {
  ...
  switch (id) {
    ...
    case "edit-player":
      ...
      if (Number.isFinite(newCash)) player.cash = newCash;
...
function loadOptions() {
  ...
  document.getElementById("correctAnswerBonus").value =
...
function applyOptions() {
  ...
  let limitScore = document.getElementById("limitScore");
  if (limitRounds.checked) {
    config.numRounds = Number.parseInt(
      document.getElementById("numRounds").value
    );
    config.winningScore = null;
  } else {
    config.numRounds = null;
    config.winningScore = Number.parseInt(
      document.getElementById("winningScore").value
    );
  }

  config.wagerChipValue1 = Number.parseInt(
    document.getElementById("wagerVal1").value
  );
  config.wagerChipValue2 = Number.parseInt(
    document.getElementById("wagerVal2").value
  );
  config.startingCash = Number.parseInt(
  ...
  else config.betLimit = null;
  if (limitBetsLastRound.checked)
    config.lastRoundBetLimit = Number.parseInt(
      document.getElementById("maxBetLastRound").value
    );
  else config.lastRoundBetLimit = null;

  config.correctAnswerBonus = Number.parseInt(
    document.getElementById("correctAnswerBonus").value
  );
  config.exactAnswerBonus = Number.parseInt(
    document.getElementById("exactAnswerBonus").value
  );
  config.centerPayout = Number.parseInt(
    document.getElementById("centerPayout").value
  );
  config.endPayout = Number.parseInt(
    document.getElementById("endPayout").value
  );
  config.elvisPayout = Number.parseInt(
    document.getElementById("elvisPayout").value
  );
